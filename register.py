...
class RegisterFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        def tao_o_nhap(tieu_de, an_chu=False, placeholder="", pad_y=(5, 12)):
        ...
        btn_dang_nhap = ctk.CTkButton(khung_chuyen, text="Đã có tài khoản? Đăng nhập ngay", font=("Arial", 13, "bold","underline"),
                                      ...
                                      width=100, cursor="hand2", command=lambda: self.controller.show_frame("LoginFrame"))
    ...
    def xu_ly_dang_ky(self):
        ...
        duong_dan_csv = os.path.normpath(os.path.join(thu_muc_hien_tai, "..", "database", "account.csv"))
        ...
        can_ghi_header = not os.path.exists(duong_dan_csv) or os.path.getsize(duong_dan_csv) == 0
        try:
            with open(duong_dan_csv, mode='a', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                if can_ghi_header:
                    # Ghi Header nếu file chưa tồn tại
                    writer.writerow(['account', 'Password', 'Role', 'Email', 'Number'])
